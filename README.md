...
You can also provide an optional `alias` parameter to choose your own custom short URL path:

```
curl -X POST \
  'https://joedelnano.com/shorten' \
  --header 'Content-Type: application/json' \
  --data-raw '{"url": "https://example.com/long-url-that-you-wish-to-shorten", "alias": "my-link"}'
```

Example response:

```
https://joedelnano.com/my-link
```

#### Use the 'short' URL

In a browser, copy and paste your shortened URL (e.g. `https://joedelnano.com/qd4azHxZgpWU`) and see the magic work!
